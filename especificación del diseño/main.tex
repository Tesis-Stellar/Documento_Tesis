...
\paragraph{Interpretación del diagrama.} El sistema central es la \textbf{plataforma Stellar Tickets}, compuesta por un frontend web (TuTicket), un backend API (NestJS) y los contratos inteligentes Soroban. Alrededor se identifican los siguientes actores y sistemas externos:
...
\begin{itemize}
  ...
  \item \textbf{Token USDC}: contrato externo de token estable utilizado como medio de pago. El contrato de evento lo invoca para ejecutar transferencias atómicas durante compras y reventas.
\end{itemize}

El límite del sistema define qué está bajo control del equipo de desarrollo (frontend, backend, contratos) y qué es externo (red Stellar, token USDC).
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.6\textwidth]{ContenedoresSecureTicket.png}
...
\begin{itemize}
  \item \textbf{Backend API}: servidor NestJS que implementa la lógica de negocio off-chain: autenticación JWT, gestión de eventos, carrito de compras con reserva temporal de asientos (\textit{seat holds}), checkout, emisión de tickets y consultas. Expone una API REST consumida por el frontend. Actúa como orquestador entre el mundo off-chain (PostgreSQL) y la blockchain (Soroban).
  \item \textbf{PostgreSQL}: base de datos relacional que persiste usuarios, eventos, sedes, boletos, órdenes y pagos. Es la fuente de datos para consultas del frontend y reportes.
...
\begin{itemize}
  ...
  \item \textbf{Contrato de Evento} (\texttt{ContratoEvento}): contrato Soroban desplegado por evento que contiene toda la lógica de negocio on-chain: creación de boletos, venta primaria, reventa atómica con burn/remint, verificación en puerta y distribución de comisiones. Es el componente más crítico del sistema.
  \item \textbf{Contrato USDC}: contrato externo del token estable utilizado como medio de pago. El contrato de evento lo invoca mediante \texttt{ClienteToken} para ejecutar transferencias atómicas.
\end{itemize}
...
\begin{itemize}
  ...
  \item El Contrato de Evento invoca al Contrato USDC para procesar pagos de forma atómica dentro de la misma transacción.
...
\textbf{Nota sobre el estado actual de integración:} la conexión Backend API $\leftrightarrow$ Soroban está diseñada pero aún no implementada. En la versión actual del prototipo, el backend simula los pagos y la emisión de boletos off-chain. La integración con los contratos inteligentes es el siguiente hito de desarrollo.
...
\begin{itemize}
  ...
  \item \textbf{Firma de transacciones:} las operaciones on-chain requieren firma criptográfica. En la arquitectura objetivo, el backend construye las transacciones Soroban y las envía a la red Stellar. La integración con wallets no-custodiales (como Freighter) queda como extensión futura para escenarios donde el usuario final firme directamente.
\end{itemize}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{@{}p{3.5cm} X@{}}
    ...
    \midrule
    \textbf{Flujo principal} &
      1. El asistente presenta su boleto (código QR o identificador).\newline
      2. El verificador consulta el estado del boleto a través del sistema.\newline
      3. El backend valida contra PostgreSQL y opcionalmente contra el contrato on-chain.\newline
      4. Si el boleto es válido, el backend invoca \texttt{redimir\_boleto} en el contrato, pasando el \texttt{ticket\_root\_id} y la dirección del verificador.\newline
      5. El contrato marca el boleto como \texttt{usado = true} (irreversible) y emite evento \texttt{BoletoRedimido}.\newline
      6. Se confirma el acceso al asistente.\\
    \midrule
    \textbf{Postcondiciones} & El boleto queda marcado como consumido on-chain. No puede ser listado, comprado ni revendido.\\
    \midrule
    \textbf{Relevancia arquitectónica} & Ejerce el modelo de autorización por verificadores (separación entre propietario y verificador), la irreversibilidad de estado on-chain y la validación Backend$\rightarrow$Soroban. La restricción de que solo verificadores autorizados pueden redimir garantiza que la redención represente un hecho físico de asistencia.\\
    \bottomrule
  \end{tabularx}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{@{}p{3cm} p{4cm} X@{}}
    ...
    Servicios & Backend API (NestJS): módulos auth, events, cart, checkout, orders, tickets, admin, users, venues & Autenticar usuarios (JWT), orquestar operaciones de negocio, gestionar carrito con seat holds, procesar checkout, exponer API REST. Futuro: orquestar transacciones Soroban.\\
    ...
    Blockchain & Contrato Factory (\texttt{FabricaBoletos}), Contrato por Evento (\texttt{ContratoEvento}), Token USDC & Crear boletos, transferir propiedad, distribuir comisiones, redimir y invalidar boletos. Fuente de verdad inmutable.\\
    \bottomrule
  \end{tabularx}
...
La comunicación con la blockchain \textbf{no es directa}: el frontend se comunica exclusivamente con el Backend API mediante llamadas REST/JSON autenticadas con JWT. El backend es el responsable de orquestar las operaciones on-chain.

\textbf{Rutas principales}: \texttt{/} (inicio), \texttt{/eventos} (catálogo), \texttt{/evento/:id} (detalle), \texttt{/evento/:id/boletas} (selección), \texttt{/carrito}, \texttt{/checkout}, \texttt{/confirmacion}, \texttt{/login}, \texttt{/registro}, \texttt{/mi-cuenta/*} (perfil, entradas, compras).

\subsection{Capa de servicios (Backend)}

El backend es una aplicación NestJS 11 (TypeScript) que centraliza la lógica de negocio off-chain. Se organiza en módulos independientes siguiendo el patrón modular de NestJS, donde cada módulo encapsula controladores, servicios y DTOs:

\subsubsection{Módulo Auth}

Gestiona registro, login y autenticación JWT con Passport.js. Define dos roles: \texttt{CUSTOMER} y \texttt{ADMIN}. Proporciona decoradores \texttt{@Public()} (rutas sin auth), \texttt{@Roles()} (restricción por rol) y \texttt{@CurrentUser()} (inyección del usuario autenticado). Las contraseñas se almacenan con hash bcrypt.

\subsubsection{Módulo Events}

Expone endpoints públicos para consultar eventos: listado con filtros, detalle por slug, eventos destacados, categorías, tipos de boletos, disponibilidad y mapa de asientos.

\subsubsection{Módulo Cart}

Gestiona el carrito de compras con reserva temporal de asientos (\textit{seat holds}) de 10 minutos de duración. Permite agregar, actualizar y remover ítems del carrito.

\subsubsection{Módulo Checkout}

Implementa el flujo de compra: previsualización del pedido (validación de carrito y cálculo de totales) y confirmación (creación de orden, registro de pago y emisión de boletos). Utiliza transacciones de base de datos (\texttt{\$transaction()}) para garantizar atomicidad.

\textbf{Nota:} en la versión actual, el pago se simula (se marca inmediatamente como \texttt{PAID}). La integración con los contratos Soroban para pagos reales on-chain es el siguiente hito de desarrollo.

\subsubsection{Módulo Tickets}

Gestiona los boletos emitidos: consulta de boletos del usuario, detalle individual y generación de payload QR (placeholder).

\subsubsection{Módulo Admin}

Panel de administración protegido por \texttt{@Roles(ADMIN)}: dashboard con estadísticas de ventas y ocupación, CRUD de eventos, creación de tipos de boletos.

\subsubsection{Módulos de soporte}

\texttt{Users} (perfil de usuario), \texttt{Venues} (sedes y secciones), \texttt{Seatmaps} (visualización de mapas de asientos), \texttt{Catalog} (servicio de catálogo).

\subsection{Capa de datos (Off-chain)}

La capa de datos off-chain se compone de una base de datos PostgreSQL 16 gestionada mediante Prisma ORM, organizada en dos schemas:

\begin{itemize}
  \item \textbf{Schema \texttt{public}}: entidades de negocio principales: \texttt{users} (con roles CUSTOMER/ADMIN), \texttt{events}, \texttt{event\_categories}, \texttt{organizers}, \texttt{cities}, \texttt{venues}, \texttt{venue\_sections}, \texttt{seats}.
  \item \textbf{Schema \texttt{ticketing}}: entidades transaccionales: \texttt{event\_ticket\_types} (tipos de boleto con precios y ventanas de venta), \texttt{event\_seat\_inventory} (estados AVAILABLE/HELD/SOLD/BLOCKED), \texttt{carts}, \texttt{cart\_items}, \texttt{seat\_holds} (reservas temporales de 10 min), \texttt{orders}, \texttt{order\_items}, \texttt{payments} (con campo \texttt{provider\_reference} reservado para tx hash blockchain), \texttt{tickets} (con estados ACTIVE/USED/CANCELLED y campo \texttt{qr\_payload}).
\end{itemize}

PostgreSQL es la fuente de datos para todas las consultas del frontend y los reportes. Cuando se complete la integración con Soroban, los datos on-chain se sincronizarán como copia indexada en estas tablas.
...
\begin{itemize}
  ...
  \item \texttt{TokenPago}: dirección del contrato de token de pago (USDC).
...
\subsubsection{Contrato de token (USDC)}

Contrato externo que implementa el estándar de token de Stellar. El contrato de evento lo invoca mediante \texttt{ClienteToken} (alias de \texttt{soroban\_sdk::token::Client}) para ejecutar las transferencias de pago. En testnet se utiliza un token USDC simulado.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{@{}X l l@{}}
    ...
    Ejecución de pagos y comisiones & \checkmark & Simulado (actual)\\
...
\begin{enumerate}
  \item El organizador ha registrado previamente uno o más verificadores autorizados invocando \texttt{agregar\_verificador(address)} en el contrato de evento.
  \item El asistente presenta su boleto (código QR o identificador) en la puerta del evento.
  \item El verificador consulta el estado del boleto a través del sistema (Backend API).
  \item El backend valida contra PostgreSQL: verifica que el boleto existe, pertenece al evento correcto, no ha sido usado ni invalidado.
  \item Si el boleto es válido, el backend invoca \texttt{redimir\_boleto(ticket\_root\_id, verificador)} en el contrato de evento.
  \item El contrato verifica que la dirección es un verificador autorizado (\texttt{Verificador(address)}), que el boleto no ha sido usado ni invalidado, y lo marca como \texttt{usado = true}, \texttt{en\_venta = false}.
  \item El contrato emite evento \texttt{BoletoRedimido} con \texttt{ticket\_root\_id}, \texttt{id\_evento}, \texttt{propietario}, \texttt{verificador} y \texttt{version}.
  \item El backend actualiza el estado en PostgreSQL y confirma el acceso al asistente.
\end{enumerate}

\textbf{Aspecto arquitectónico clave:} la separación entre propietario y verificador es una decisión de diseño que garantiza que la redención represente un hecho físico de asistencia. El propietario tiene custodia del boleto, pero no puede auto-redimirlo remotamente. Solo un verificador autorizado por el organizador (registrado on-chain) puede ejecutar la redención, proporcionando trazabilidad de quién verificó y cuándo.
...
El sistema se despliega en cuatro nodos lógicos que pueden residir en infraestructura compartida o independiente.
...
\begin{tabularx}{\textwidth}{l l X}
\toprule
\textbf{Nodo} & \textbf{Tecnología} & \textbf{Componentes} \\
\midrule
Cliente & Navegador web & Aplicación React (TuTicket) servida como SPA estática. \\
Servidor de aplicación & Node.js 20 & Backend NestJS, servidor Vite de desarrollo (o archivos estáticos en producción). \\
Servidor de datos & PostgreSQL 16 & Esquemas \texttt{public} (autenticación, usuarios) y \texttt{ticketing} (eventos, boletos, carritos). \\
Blockchain & Stellar / Soroban & Contratos \texttt{FabricaBoletos} y \texttt{ContratoEvento} desplegados en Testnet. \\
\bottomrule
\end{tabularx}
\end{table}

\subsection{Configuración de contenedores}

El backend utiliza Docker Compose para orquestar los servicios en desarrollo:

\begin{itemize}
  \item \textbf{PostgreSQL 16}: contenedor oficial \texttt{postgres:16}, puerto 5432, volumen persistente para datos.
  \item \textbf{Backend NestJS}: ejecutado con \texttt{npm run start:dev} (modo watch). Conecta a PostgreSQL vía Prisma.
  \item \textbf{Frontend Vite}: ejecutado con \texttt{npm run dev}, proxy configurado hacia el backend en desarrollo.
\end{itemize}
...
\midrule
\texttt{DATABASE\_URL} & Backend & Cadena de conexión PostgreSQL (Prisma). \\
\texttt{JWT\_SECRET} & Backend & Secreto para firma de tokens JWT. \\
\texttt{SOROBAN\_RPC\_URL} & Backend & URL del nodo RPC de Soroban (futuro). \\
\texttt{FACTORY\_CONTRACT\_ID} & Backend & ID del contrato \texttt{FabricaBoletos} desplegado (futuro). \\
\texttt{VITE\_API\_URL} & Frontend & URL base del backend API. \\
\bottomrule
\end{tabularx}
\end{table}

% Diagrama de despliegue (compilar externamente e incluir como imagen)
% \begin{figure}[H]
%   \centering
%   \includegraphics[width=0.95\textwidth]{despliegue.png}
...
El proyecto se distribuye en cuatro repositorios Git independientes:
...
\midrule
\texttt{main\_contract} & \texttt{codigo/main\_contract/} & Contratos inteligentes Soroban (Rust). \\
\texttt{Tu-Ticket-Frontend} & \texttt{codigo/Tu-Ticket-Frontend/} & Aplicación web React + Vite + TypeScript. \\
\texttt{Tu-Ticket-Backend} & \texttt{codigo/Tu-Ticket-Backend/} & API REST NestJS + Prisma + PostgreSQL. \\
...
\midrule
Contratos & \texttt{make build} & \texttt{target/wasm32-.../\{event,factory\}\_contract.wasm} \\
...
Tests contratos & \texttt{make test} & Ejecución de \texttt{cargo test} \\
\bottomrule
\end{tabularx}
\end{table}
...
La base de datos relacional utiliza dos esquemas Prisma que mapean a PostgreSQL 16:

\subsubsection{Esquema \texttt{public} (autenticación)}

\begin{itemize}
  \item \texttt{User}: id, email (único), password (hash bcrypt), nombre, rol (\texttt{USER} | \texttt{ADMIN}), timestamps.
  \item \texttt{RefreshToken}: id, token, userId (FK), expiresAt.
\end{itemize}

\subsubsection{Esquema \texttt{ticketing} (dominio de negocio)}

\begin{itemize}
  \item \texttt{Event}: id, nombre, descripción, fecha, lugar, capacidad, precio base, organizadorId (FK), estado, imágenes, timestamps.
  \item \texttt{Ticket}: id, eventId (FK), userId (FK), estado (\texttt{AVAILABLE} | \texttt{SOLD} | \texttt{USED} | \texttt{CANCELLED}), precio, asiento, timestamps.
  \item \texttt{Cart}: id, userId (FK), estado, expiresAt, timestamps.
  \item \texttt{CartItem}: id, cartId (FK), ticketId (FK), precio, timestamps.
  \item \texttt{Purchase}: id, userId (FK), total, estado, transactionRef, timestamps.
  \item \texttt{PurchaseItem}: id, purchaseId (FK), ticketId (FK), precio.
\end{itemize}
...
\midrule
Evento & \texttt{ContratoEvento} (contrato hijo) & Tabla \texttt{Event} \\
Boleto & \texttt{Boleto} struct en storage & Tabla \texttt{Ticket} \\
Propietario & \texttt{propietario} (Address Stellar) & \texttt{userId} (FK a User) \\
Estado de uso & \texttt{usado} / \texttt{invalidado} (bool) & \texttt{estado} (enum) \\
Compra & Transferencia atómica de tokens & Tabla \texttt{Purchase} \\
Comisiones & Split automático en \texttt{comprar\_boleto} & No almacenado (derivable) \\
\bottomrule
\end{tabularx}
\end{table}

\textbf{Nota:} En el estado actual del desarrollo, la integración Soroban no está implementada. Los pagos se simulan en el backend. La correspondencia on-chain/off-chain se materializará cuando se conecte el backend con los contratos desplegados en Testnet.
...
Artefacto & Contratos Soroban + Backend NestJS. \\
...
\textbf{Táctica:} On-chain, cada función pública usa \texttt{require\_auth()} sobre la dirección correspondiente (organizador, propietario, verificador, admin). Off-chain, NestJS usa guards JWT con decorador \texttt{@Roles()} y middleware de autenticación.
...
Artefacto & Frontend React, Backend NestJS. \\
...
Artefacto & Backend NestJS + PostgreSQL. \\
...
\textbf{Decisión:} Se mantiene on-chain únicamente la lógica crítica de negocio (propiedad, transferencias, comisiones, redención). La gestión de usuarios, catálogo de eventos, carrito de compras, y lógica de presentación vive off-chain en NestJS + PostgreSQL.

\textbf{Consecuencias:} Mejor experiencia de usuario (operaciones off-chain son instantáneas). Menor costo de gas. Complejidad adicional para mantener consistencia entre ambos mundos (se resolverá con indexación de eventos on-chain cuando se complete la integración).
...
\textbf{Decisión:} Frontend con React 18 + Vite + TypeScript + Tailwind CSS + shadcn/ui. Backend con NestJS 11 + TypeScript + Prisma ORM + PostgreSQL 16. Autenticación con JWT (access + refresh tokens).

\textbf{Consecuencias:} TypeScript compartido entre frontend y backend reduce el cambio de contexto. Prisma proporciona migraciones type-safe y genera tipos automáticamente. shadcn/ui acelera el desarrollo de UI con componentes accesibles. NestJS ofrece arquitectura modular que escala bien con la complejidad del dominio.
