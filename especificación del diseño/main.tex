...
% ====== Permitir corte de línea dentro de \texttt (rutas/identificadores) ======
% Evita que texto monoespaciado se salga del margen derecho. Inserta puntos
% de corte (sin guion) que solo se usan si la línea lo necesita:
%   - tras separadores ASCII  / . : - ,  y  \_   con prioridad alta (pen. 0)
%   - tras cada letra/dígito ASCII con prioridad baja (pen. 800), como
%     último recurso para cadenas largas sin separadores (p. ej.
%     direcciones de contrato base32).
% No toca caracteres acentuados (UTF-8): no se desfasa la codificación.
\makeatletter
\let\origtexttt\texttt
% Define el significado "activo" del carácter #1 (carácter + punto de corte)
% sin alterar su catcode global; el catcode se activa solo dentro de \texttt.
\def\ttbreak@def#1{%
  \expandafter\edef\csname ttbk@\number`#1\endcsname{%
    \noexpand\char\number`#1\relax
    \noexpand\penalty\ttbreak@pen\noexpand\hskip0pt\noexpand\relax}%
  \expandafter\let\expandafter\ttbreak@cs\csname ttbk@\number`#1\endcsname
  \begingroup\lccode`\~=`#1\relax
  \lowercase{\endgroup\global\let~\ttbreak@cs}%
}
% Separadores: prioridad alta (penalty 0)
\def\ttbreak@pen{0}%
\ttbreak@def/\ttbreak@def.\ttbreak@def:\ttbreak@def-\ttbreak@def,
% Letras y dígitos ASCII: prioridad baja (penalty 800)
\def\ttbreak@pen{800}%
\@tfor\ttbreak@c:=%
ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789\do{%
  \expandafter\ttbreak@def\ttbreak@c}%
% Versión de \_ con punto de corte (penalty 0)
\def\ttbreak@uscore{\char95\penalty0\hskip0pt\relax}%
\def\ttbreak@actcat#1{\catcode`#1=\active}%
% Activa los catcodes solo dentro del grupo de \texttt
\def\ttbreak@chars{%
  \catcode`\/=\active \catcode`\.=\active \catcode`\:=\active
  \catcode`\-=\active \catcode`\,=\active
  \@tfor\ttbreak@c:=%
ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789\do{%
    \expandafter\ttbreak@actcat\ttbreak@c}%
  \let\_\ttbreak@uscore}%
\DeclareRobustCommand{\texttt}{\begingroup\ttfamily\ttbreak@chars\ttbreak@grab}
\def\ttbreak@grab#1{#1\endgroup}
\makeatother
% Relaja la justificación para reducir desbordes en párrafos
...
\begin{itemize}
  ...
  \item \textbf{Contrato de Evento} (\texttt{ContratoEvento}, 12 instancias desplegadas): contrato Soroban desplegado por evento que contiene toda la lógica de negocio on-chain: creación de boletos, venta primaria, reventa atómica con burn/remint, distribución de comisiones y gestión de verificadores. Es el componente más crítico del sistema.
...
\begin{table}[h!]
  ...
  \begin{tabularx}{\textwidth}{@{}p{3.5cm} X@{}}
    ...
    \midrule
    \textbf{Relevancia arquitectónica} & Ejerce la orquestación Backend$\rightarrow$Soroban, la escritura on-chain con registro off-chain posterior, y la transición \texttt{es\_reventa = false -> true} que determina el comportamiento de todas las transacciones futuras sobre ese boleto.\\
    \bottomrule
  \end{tabularx}
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{@{}p{3cm} p{4cm} X@{}}
    ...
    Blockchain & Contrato Factory (\texttt{FabricaBoletos}), Contrato por Evento (\texttt{ContratoEvento}), Contrato NFT (\texttt{ticket\_nft\_contract}), SAC XLM nativo & Crear boletos, transferir propiedad, distribuir comisiones, redimir e invalidar boletos. Fuente de verdad inmutable. 12 contratos de evento desplegados en Stellar Testnet.\\
    \bottomrule
  \end{tabularx}
...
\begin{enumerate}
  ...
  \item \textbf{Ejecución atómica en el contrato:} como \texttt{es\_reventa = true}, el contrato ejecuta la distribución de comisiones y el burn/remint en una sola transacción:
    \begin{enumerate}[label=\alph*)]
      \item Calcula la comisión del organizador: \texttt{precio * ComisionOrganizador / 100}.
      \item Calcula la comisión de la plataforma: \texttt{precio * ComisionPlataforma / 100}.
...
Blockchain & Stellar Testnet (Soroban) & \texttt{FabricaBoletos} + 12 instancias de \texttt{ContratoEvento} + \texttt{ticket\_nft\_contract} + SAC XLM nativo. \\
\bottomrule
\end{tabularx}
\end{table}
...
El repositorio de contratos utiliza un workspace de Cargo con tres crates:
...
+-- contracts/
    ...
    +-- ticket_nft_contract/
        +-- src/
            +-- lib.rs       # Contrato NFT SEP-41
            +-- test.rs      # Suite de tests
\end{verbatim}

Dependencias: \texttt{factory\_contract} depende de \texttt{event\_contract} (importa \texttt{ContratoEventoClient} para inicializar contratos hijos) y el contrato de evento integra la semántica NFT expuesta por \texttt{ticket\_nft\_contract}.
...
\midrule
Contratos & \texttt{stellar contract build} & \texttt{event\_contract.wasm}, \texttt{factory\_contract.wasm} y \texttt{ticket\_nft\_contract.wasm} en \texttt{target/wasm32v1-none/release/} \\
