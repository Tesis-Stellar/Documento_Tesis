...
incrementar el costo efectivo, comportamiento que no se observó durante la evaluación; y (iv) la verificación
en puerta resuelve contra la proyección relacional mediante \texttt{POST /api/admin/scan} y no contra el
contrato en cada escaneo, lo que traslada momentáneamente la confianza al estado off-chain del indexador
durante la ventana de sincronización; la \textit{redención on-chain} se mantiene como respaldo de auditoría
pero no se ejecuta en la ruta crítica de baja latencia (esta vulnerabilidad y sus mitigaciones se detallan en
